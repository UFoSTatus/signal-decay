...
// ◆◆◆ REAL-WORLD CLINICAL ENHANCEMENT — 2026-09-07 ◆◆◆

// Additive pass grounding the memory system in real trauma neuroscience.

// NOTHING removed — only new sections and accessors added. New top-level keys:

//   • NEUROSCIENCE_NOTES              — dual-representation theory, hippocampus/amygdala

//                                       split, state-dependent memory, flashback mechanics,

//                                       dreams, childhood encoding, false-memory caution

//   • RECOVERY_METHOD_CLINICAL_BASIS  — evidence tier + real mechanism + real risk for

//                                       each RECOVERY_METHODS entry (EMDR/PE, SE, hypnosis…)

//   • REALISTIC_SYMPTOM_PROGRESSION   — fragment→narrative stages, Herman's 3-stage model,

//                                       "the body keeps the score", real recovery timelines

//   • MEMORY_CLINICAL_GROUNDING       — per-memory clinical notes (all 25 memories)

// New api accessors: getNeuroscienceNotes, getMethodClinicalBasis, getSymptomProgression,

//   getMemoryClinicalGrounding, getMemoryWithClinicalGrounding.

// Sources: Brewin, van der Kolk, Herman, Radulovic, Ehlers & Clark, Loftus, Levine, Walker.
...
const MEMORY_RECOVERY_ENGINE = {
  ...
  // NEUROSCIENCE NOTES  ◆ Real-World Enhancement — added 2026-09-07

  // ═══════════════════════════════════════════════════════════════════════════

  // Clinical grounding for the SUPPRESSED→FRAGMENTS→SURFACING→RECOVERED model.

  // ADDITIVE: explains WHY the game states behave as they do so writers keep

  // symptom depictions clinically plausible. Sources in NEUROSCIENCE_NOTES.citations.



  NEUROSCIENCE_NOTES: {

    description: 'Real neurological mechanisms underlying the four-state memory model. Suppression is a RETRIEVAL failure, not a STORAGE failure — the data is intact; the brain cannot reach it because it was filed under a neurochemical state normal waking consciousness cannot reproduce.',

    core_principle: 'Traumatic memories are NOT deleted. They are encoded differently — fragmented, sensory-dominant, non-narrative. The body has the raw sensory-emotional data but cannot construct a narrative around it.',



    dual_representation_theory: {

      source: 'Brewin, 2001 & 2014 — Dual-Representation Theory of PTSD',

      VAM: 'Verbally Accessible Memory — normal autobiographical memory. Narrative, sequential, consciously retrievable, placed in time/context. In trauma this system is SUPPRESSED.',

      SAM: 'Situationally Accessible Memory — sensory/emotional/physiological fragments (images, sounds, smells, body states). NOT consciously retrievable; triggered involuntarily by cues matching original encoding conditions. In trauma this system DOMINATES.',

      game_mapping: 'SUPPRESSED and FRAGMENTS are pure SAM (body reactions, sensory flashes, no story). SURFACING is where SAM fragments begin re-linking to VAM. RECOVERED is full VAM reconstruction with the emotional charge metabolized.',

    },



    dissociative_encoding_hypothesis: {

      source: 'Ehlers & Clark, 2000; Foa & Riggs, 1993; Bedard-Gilligan & Zoellner, 2012 (PMC3310188)',

      mechanism: 'Peritraumatic dissociation (dissociating DURING the event) prevents proper hippocampal encoding while the amygdala keeps encoding emotional/threat data independently. Result: emotional memories with no narrative structure — the feeling exists, the story does not.',

    },



    hippocampus_amygdala_split: {

      source: 'Brown & Morey, 2012; Shin et al., 2006 (neuroimaging in PTSD)',

      hippocampus: 'Contextual encoding — the what/where/when. SUPPRESSED by stress hormones; high cortisol impairs it. Output = the STORY (temporal sequence, autobiographical narrative).',

      amygdala: 'Emotional/threat encoding — how dangerous is this. ENHANCED by stress hormones; norepinephrine supercharges it. Output = the FEELING (raw terror, physiological emergency).',

      result: 'Intense emotional/physiological responses with no coherent narrative to attach them to. This is the neurobiological basis of flashbacks: vivid re-experiencing of sensory/emotional fragments with no temporal context — the brain literally cannot tell the event is in the past.',

      game_mapping: 'Characters should experience WHAT IT FELT LIKE (amygdala) long before they learn WHAT HAPPENED (hippocampal narrative). A memory surfaces as "overwhelming terror tied to a red light" before it becomes "the night the facility alarm went off."',

    },



    state_dependent_memory: {

      source: 'Radulovic, Lee & Ortony, 2018 (Frontiers in Behavioral Neuroscience); Patient X case',

      mechanism: 'Memories encoded under extreme stress (high cortisol/norepinephrine, altered GABA signaling, a theta→gamma oscillation shift) are inaccessible in calm states and become accessible only when a similar neurochemical state is re-entered.',

      implication: 'Trauma often surfaces during NEW stress because the neurobiological state matches the original encoding. To recover a memory the character may need to re-approach the emotional state of the event — carefully, to avoid retraumatization.',

      game_mapping: 'Justifies the PRESSURE_SYSTEM EMOTIONAL_STATE and FREQUENCY sources: matching the original state (or its frequency signature) is literally how the file gets found again.',

    },



    olfactory_and_sensory_triggers: {

      source: 'Bremner et al., 2004 (PMC3236699); Wilson & Stevenson, Memory and Plasticity in the Olfactory System',

      smell: 'The olfactory bulb connects DIRECTLY to amygdala and hippocampus — the only sensory system with such direct limbic access (no thalamic relay). Smell triggers are the most powerful and most resistant to extinction; they hold their emotional charge longest.',

      potency_hierarchy: 'Clinical trigger potency ranks roughly: OLFACTORY > AUDITORY (tone/voice quality/rhythm) ≈ SOMATIC (touch/pressure/temperature) > VISUAL (light quality/spatial config) > TASTE.',

      game_mapping: 'When a memory has an OLFACTORY fragment_trigger, treat it as the highest-intensity vector. Sensory cues should produce the physiological/emotional response BEFORE any narrative surfaces.',
    ...
    flashback_mechanics: {
      ...
      source: 'Ehlers & Clark, 2000; Brewin, 2015 — intrusive re-experiencing vs. narrative recall',

      step_1_cue_detection: 'A stimulus matches a stored trauma association; the amygdala processes the match in ~12ms — far faster than conscious awareness (~200-500ms).',

      step_2_amygdala_hijack: 'Amygdala fires a threat response before the prefrontal cortex can evaluate. HPA axis → cortisol; sympathetic surge → norepinephrine; fight/flight/freeze cascade (HR up, BP up, muscle tension, pupil dilation).',

      step_3_hippocampal_failure: 'The norepinephrine/cortisol surge suppresses the hippocampus, so the memory is experienced WITHOUT time-stamping — it feels present-tense.',

      step_4_perceptual_reexperiencing: 'Sensory cortices reactivate (replay stored sensory data); motor areas fire (protective postures); the insula floods with original body states; the anterior cingulate (which distinguishes memory from perception) is suppressed.',

      step_5_nowness: 'The defining feature: the person is not REMEMBERING the trauma, they are RE-EXPERIENCING it. Time distortion is common — a 2-second flashback can feel like minutes.',

      game_mapping: 'SURFACING-state flashbacks must feel qualitatively different from ordinary recall: sensory, present-tense, overwhelming, fragmentary, involuntary — a frozen moment, not a tidy scene. Cues can trigger flashbacks the character does not consciously recognize as related.',
    ...
    dreams_as_channel: {
      ...
      source: 'Walker & van der Helm, 2009 (Sleep-to-Forget/Sleep-to-Remember); Germain, 2013; Pace-Schott et al., 2015',

      mechanism: 'REM normally consolidates the FACTS of an emotional experience while stripping away its emotional CHARGE (decoupling memory from arousal). In PTSD this fails — fragmented REM never strips the charge, so each re-encounter carries full original intensity. Dreams bypass prefrontal executive control (the gatekeeper), letting suppressed material surface.',

      reliability_caveat: 'Dream CONTENT is emotionally meaningful but factually UNRELIABLE — it blends real fragments with symbolic/imaginative material. Replicative nightmares (near-exact replays) are the exception and are a PTSD hallmark.',

      game_mapping: 'DREAM_INVASION should present distorted, symbolic, fragmentary imagery the player must interpret — connecting to real fragments that later surface in waking gameplay. Never treat a dream as a clean, trustworthy recording.',
    ...
    childhood_encoding: {
      ...
      source: 'Frankland et al., 2013 (postnatal hippocampal neurogenesis); developmental HPA-axis literature',

      infantile_amnesia: 'Before ~age 2-3 the hippocampus cannot form retrievable episodic memories; postnatal neurogenesis physically OVERWRITES early synaptic traces. This is development, NOT repression. A detailed narrated "memory" from infancy is clinically implausible and should be flagged as potentially constructed.',

      ages_3_to_7: 'Memories from ages 3-7 are potentially recoverable but fragmentary, emotionally distorted, and blended with later experience/suggestion. Children encode trauma as procedural patterns (flinching), emotional schemas (all authority = danger), and somatic states — NOT clean scenes with dialogue.',

      game_mapping: 'CHILDHOOD_VISIT_CLUSTER memories should surface primarily as body sensations, emotional states, behavioral urges, and fragmentary images — not full scenes. The engine already gates these behind Tier 1-3 partial awakening, mirroring the clinical reality that early material needs scaffolding to approach.',

      in_world_exception: 'In Signal Decay the Overtone/soul mechanism is the FICTIONAL reason a few childhood events retain more vivid encoding than biology alone allows — treat that as the deliberate supernatural exception to the real rule above.',

    },



    false_memory_caution: {

      source: 'Loftus (Lost in the Mall, 1995); Orne et al., 1988; APA guidance on hypnosis',

      principle: 'Recovered material can be genuine, confabulated, or a blend. Suggestive questioning, hypnosis, and social corroboration all raise false-memory risk. Clinical consensus is BALANCED: real recovered memories exist AND false memories can be implanted.',

      game_mapping: 'The unreliability is a narrative asset. Lower-evidence methods (HYPNOTIC_REGRESSION, DREAM_INVASION) yield deliberately ambiguous content; higher-evidence pathways (processing known material) yield trustworthy recall.',
    ...
    citations: [
      ...
      'Brewin, C.R. (2001, 2014). Dual-Representation Theory of PTSD.',

      'Ehlers, A. & Clark, D.M. (2000). A cognitive model of PTSD.',

      'Radulovic, J., Lee, R. & Ortony, A. (2018). State-dependent memory. Frontiers in Behavioral Neuroscience.',

      'Bremner, J.D. et al. (2004). Neural correlates of trauma-related odor cues. PMC3236699.',

      'Walker, M.P. & van der Helm, E. (2009). Overnight therapy? The role of sleep in emotional processing.',

      'van der Kolk, B. (1994, 2014). The Body Keeps the Score.',

      'Frankland, P.W. et al. (2013). Neurogenesis and infantile amnesia.',

      'Herman, J. (1992). Trauma and Recovery.',

      'Loftus, E. (1995). The formation of false memories (Lost in the Mall).',

      'Bedard-Gilligan, M. & Zoellner, L.A. (2012). Dissociation and memory fragmentation in PTSD. PMC3310188.',
  ...
  // CLINICAL BASIS — Recovery Methods  ◆ Real-World Enhancement — added 2026-09-07

  // ═══════════════════════════════════════════════════════════════════════════

  // Evidence-based notes for each entry in RECOVERY_METHODS. Maps each in-game

  // method to its closest real-world therapy, its evidence tier, its real

  // mechanism, and its real risks. ADDITIVE — does not alter RECOVERY_METHODS.

  // Evidence tiers: STRONG (guideline-recommended) → MODERATE (promising/controversial)

  // → WEAK/CONTESTED → EXPERIMENTAL/SPECULATIVE.



  RECOVERY_METHOD_CLINICAL_BASIS: {

    description: 'Real-world grounding for each recovery method. Keyed to RECOVERY_METHODS ids. Use these notes to keep the in-game portrayal honest about what actually works, how, and at what cost.',



    CASUAL_CONVERSATION: {

      real_world_analog: 'Supportive/relational disclosure & narrative prompting; the front end of Trauma-Focused CBT and Prolonged Exposure narration.',

      evidence_tier: 'STRONG (as part of TF-CBT / PE narrative work)',

      real_mechanism: 'Gentle, non-suggestive prompting lets the person begin narrating known material; free recall can surface previously unrecognized connections without forcing content. Safety and trust (Herman Stage 1) are prerequisites.',

      what_actually_happens: 'No memory is "extracted." A trusted listener lowers defensive arousal enough that a fragment the person already half-holds can be spoken. Effective for early SUPPRESSED→FRAGMENTS movement only.',

      real_risks: 'Low. Main risk is leading questions introducing false detail — keep prompts open-ended and non-suggestive.',

      false_memory_risk: 'LOW if non-suggestive; MODERATE if the questioner supplies specifics.',

      clinical_caveat: 'Cannot force deep or state-dependent memories — realistically caps at fragment-level surfacing, exactly as the engine models (SUPPRESSED_TO_FRAGMENTS).',
    ...
    GUIDED_MEDITATION: {
      ...
      real_world_analog: 'Mindfulness / body-scan meditation; Somatic Experiencing (Levine) titration & pendulation.',

      evidence_tier: 'MODERATE (body-scan comparable to EMDR/Brainspotting for distress reduction — D\'Antoni et al., 2022)',

      real_mechanism: 'Sustained non-judgmental attention lowers defensive self-monitoring and hyperarousal, creating conditions where avoided internal material surfaces spontaneously. Somatic Experiencing tracks body sensations (SIBAM: Sensation-Image-Behavior-Affect-Meaning) and reconnects decoupled channels gradually via titration (small doses) and pendulation (oscillating between distress and calm/resource).',

      what_actually_happens: 'Not a retrieval technique per se — it reduces the barrier so SAM fragments can rise. Especially suited to body-memory-first material (childhood, preverbal).',

      real_risks: 'Generally low, BUT intensive meditation can trigger unexpected traumatic emergence and psychological crisis in trauma survivors — flooding without a resource anchor is the danger. Model this as the "emotional overwhelm" physical_cost already listed.',

      false_memory_risk: 'LOW-MODERATE.',

      clinical_caveat: 'Best paired with a stabilization/grounding step first (Herman Stage 1). Fits the engine\'s SUPPRESSED→FRAGMENTS and FRAGMENTS→SURFACING transitions.',
    ...
    HYPNOTIC_REGRESSION: {
      ...
      real_world_analog: 'Hypnotic age regression / hypnotic memory retrieval.',

      evidence_tier: 'WEAK / CONTESTED — contraindicated for memory retrieval by APA, BPS and other bodies.',

      real_mechanism: 'Hypnosis increases SUGGESTIBILITY and CONFIDENCE, not accuracy. It can access dissociative states and reduce arousal enough to approach material, but it does not reliably recover true memories — and it can implant false ones (Orne et al., 1988; Leo, Bruno & Proietti, 2025).',

      what_actually_happens: 'People produce MORE detail under hypnosis, but the extra detail is disproportionately confabulated. Hypnotically "recovered" memories are inadmissible as evidence in many jurisdictions.',

      real_risks: 'HIGH false-memory risk; risk of iatrogenic (therapy-induced) pseudo-memories; retraumatization if arousal spikes.',

      false_memory_risk: 'VERY HIGH — this unreliability is a deliberate story asset.',

      clinical_caveat: 'Legitimate uses are anxiety/pain/habit and creating conditions for processing KNOWN material — NOT recovering unknown memories. In-game, hypnotic-regression content should be deliberately ambiguous about what is real vs. suggestion-generated. The engine\'s 30% success rate correctly reflects its unreliability.',
    ...
    FREQUENCY_EXPOSURE: {
      ...
      real_world_analog: 'Prolonged Exposure (PE) + EMDR bilateral processing; the fictional "frequency" stands in for direct, controlled re-exposure to the encoded stimulus.',

      evidence_tier: 'STRONG (PE and EMDR are guideline first-line for PTSD)',

      real_mechanism: 'Controlled re-exposure to the trauma stimulus while the memory is held in a safe context drives reconsolidation — the memory transfers from implicit/present-tense storage to explicit/past-tense narrative, shedding emotional charge. EMDR bilateral stimulation may tax working memory (reducing vividness) and mimic REM emotional processing. In Patient X, PE finally enabled full recovery after 15 years.',

      what_actually_happens: 'Direct exposure produces the fullest recall of any real method but at the cost of intense in-session activation. In the fiction the "frequency" is the exact encoding stimulus (state-dependent key), which is why exposure detonates rather than gently surfaces.',

      real_risks: 'HIGH activation: full autonomic surge, dissociation, retraumatization if done without stabilization. Real PE/EMDR require trained clinicians and titration — the engine\'s "physical damage / hospitalization" cost is the dramatized version.',

      false_memory_risk: 'LOW when non-suggestive (real strength of exposure-based methods).',

      clinical_caveat: 'Highest fidelity recall, highest activation cost — matches the engine\'s 85% success + SEVERE physical_cost. Should never be a first move on an unstabilized character.',
    ...
    NEAR_DEATH_EXPERIENCE: {
      ...
      real_world_analog: 'Recalled Experiences of Death (REDs) / "panoramic life review".',

      evidence_tier: 'EXPERIMENTAL / SPECULATIVE',

      real_mechanism: 'The dying brain shows surges of gamma oscillations in memory-consolidation regions (2023 study); life-review phenomena involve massive hippocampal/default-mode activation. NDE memories test as detail-rich, self-referential and emotional — distinct from imagined memories (Frontiers in Human Neuroscience, 2014).',

      what_actually_happens: 'A flood of autobiographical content, including material previously inaccessible, can enter consciousness during the dying process. Real neuroscience partially supports the "everything floods at once" trope.',

      real_risks: 'By definition life-threatening; not a therapeutic technique. Content richness does not guarantee accuracy.',

      false_memory_risk: 'UNKNOWN / MIXED.',

      clinical_caveat: 'Strictly a narrative event, as the engine already marks it — 100% recall but at ANY_TO_RECOVERED with life-threatening cost. Do not present as a real therapy.',
    ...
    DREAM_INVASION: {
      ...
      real_world_analog: 'Dream/nightmare processing; REM emotional-memory reprocessing (IRT — Imagery Rehearsal Therapy adjacent).',

      evidence_tier: 'WEAK / CONTESTED as a memory-RETRIEVAL method (dreams are not archived records).',

      real_mechanism: 'REM bypasses prefrontal executive control and keeps the amygdala active, so suppressed material can surface — but in distorted, symbolic form. In PTSD, replicative nightmares can replay events near-exactly (Walker & van der Helm, 2009; Germain, 2013).',

      what_actually_happens: 'Dreams surface emotionally meaningful fragments, not reliable footage. They blend real trace with symbol. Sleep disturbance both signals and worsens trauma.',

      real_risks: 'Nightmares, sleep disruption, next-day dysregulation; content must not be treated as literal truth.',

      false_memory_risk: 'MODERATE — dream content is factually unreliable; treat as a lead, not proof.',

      clinical_caveat: 'Ideal for surfacing fragments that must be interpreted and later corroborated in waking gameplay. Matches the engine\'s 40% success and SUPPRESSED→FRAGMENTS / FRAGMENTS→SURFACING transitions.',

    },



    CROSS_CHARACTER_RESONANCE: {

      real_world_analog: 'Group trauma therapy; social contagion of memory; the "I thought I was the only one" recognition moment.',

      evidence_tier: 'MODERATE (well-documented group dynamic — Hirst & Echterhoff, 2012; Herman, 1992; van der Kolk, 2014)',

      real_mechanism: 'When one survivor recovers a fragment, similarity can catalyze recovery in others who shared the event (emotional contagion + validation collapsing the isolation that maintained amnesia). BUT collaborative recall converges narratives and can import false detail; social trust lowers the barrier to accepting a suggested memory.',

      what_actually_happens: 'Shared testimony is one of the most powerful catalysts in trauma recovery AND one of the biggest false-memory hazards. Different people encode the SAME event differently, so divergent versions are expected and may be irreconcilable.',

      real_risks: 'Social pressure to produce group-matching memories; cross-contamination of detail; simultaneous destabilization of multiple people.',

      false_memory_risk: 'MODERATE-HIGH via collaborative recall distortion.',

      clinical_caveat: 'Model divergent versions of shared scenes that must be reconciled (or left irreconcilable). Matches the engine\'s FRAGMENTS→SURFACING transition and the cluster resonance-pressure system.',
    ...
    method_selection_guidance: {
      ...
      stabilize_first: 'Herman Stage 1 (Safety & Stabilization) must precede deep memory work. Using FREQUENCY_EXPOSURE, HYPNOTIC_REGRESSION, or NEAR_DEATH_EXPERIENCE on an unstabilized character is the in-fiction equivalent of contraindicated clinical practice — and should carry the worst outcomes.',

      evidence_ranking: 'By real evidence: STRONGEST = FREQUENCY_EXPOSURE (PE/EMDR analog) and CASUAL_CONVERSATION/GUIDED_MEDITATION front-ends; CONTESTED = HYPNOTIC_REGRESSION, DREAM_INVASION; SPECULATIVE = NEAR_DEATH_EXPERIENCE. Note this is deliberately INVERTED from in-game success_rate for dramatic effect — the most reliable in-fiction method (frequency) is also the most dangerous.',

      reliability_vs_safety: 'Real recovery trades reliability against activation cost: gentle methods are safe but shallow; exposure is deep but activating; hypnosis/dreams are dramatic but untrustworthy. The engine\'s difficulty/risk/success spread already encodes this trade-off.',
  ...
  // REALISTIC SYMPTOM PROGRESSION  ◆ Real-World Enhancement — added 2026-09-07

  // ═══════════════════════════════════════════════════════════════════════════

  // Maps the four game STATES onto the real clinical fragment-to-narrative

  // progression and Herman's three-stage recovery model, with realistic body

  // symptoms (van der Kolk) and real timelines. ADDITIVE reference layer —

  // does not modify STATES, PRESSURE_SYSTEM, or any memory object.



  REALISTIC_SYMPTOM_PROGRESSION: {

    description: 'Clinically-grounded progression the four game states are modeled on. Suppressed memories return body-first, then sensory, then emotional, then partial scene, then narrative — NOT as a clean unlock. Progression is NON-LINEAR: stress/triggers can regress a memory to an earlier stage or catalyze a sudden jump.',



    fragment_to_narrative_stages: {

      source: 'Brewin 2001; Ehlers & Clark 2000; van der Kolk 1994; Levine 2015; Herman 1992',

      note: 'These five clinical stages (A-E) sit UNDERNEATH the four game states. The engine compresses them: STATE mapping given per stage.',



      stage_A_sensory_fragments: {

        maps_to_game_state: 'FRAGMENTS (entry)',

        timeline_real: 'Days to weeks',

        what_surfaces: 'Isolated sensory flashes — a color, a sound, a smell, a texture, a taste. Disconnected from any narrative; the person does not know what they are remembering.',

        emotional_tone: 'Free-floating anxiety/dread/startle — OR emotionally flat (dissociated).',

        body_response: 'Localized sensations — heat, cold, pressure, pain — in specific body areas.',

        example_line: '"I keep seeing a red light. I don\'t know why. It just appears in my mind."',

      },



      stage_B_emotional_recovery: {

        maps_to_game_state: 'FRAGMENTS (deepening) → SURFACING',

        timeline_real: 'Weeks to months',

        what_surfaces: 'The EMOTION of the original event returns — often BEFORE any factual content. Overwhelming feeling with no attachable event.',

        emotional_tone: 'Intense, destabilizing, "feels crazy" — "why am I so afraid? Nothing happened."',

        body_response: 'Full autonomic activation — fight/flight/freeze physiology; racing heart, urge to flee.',

        example_line: '"I\'m terrified every time I enter that room. My heart pounds. I feel like I need to run. I can\'t explain it."',

      },



      stage_C_partial_narrative: {

        maps_to_game_state: 'SURFACING',

        timeline_real: 'Months',

        what_surfaces: 'Incomplete scenes — frozen images, short sequences, fragments of dialogue. Like a corrupted film: some frames present, others missing, possibly out of order or distorted.',

        emotional_tone: 'Emotion begins connecting to specific content; moments of "oh god, something happened."',

        body_response: 'Specific somatic memories emerge — "my wrist hurts when I think about it."',

        example_line: '"I can see the hallway. Someone at the end of it. I can\'t see their face. Then it just... stops."',

      },



      stage_D_narrative_assembly: {

        maps_to_game_state: 'SURFACING → RECOVERED',

        timeline_real: 'Months to years',

        what_surfaces: 'The story coheres. Fragments connect; temporal sequence establishes. Missing pieces may fill in or remain permanently absent.',

        emotional_tone: 'Grief, mourning — the full weight of what happened becomes real.',

        body_response: 'Activation begins to organize around the now-known narrative rather than firing randomly.',

        example_line: '"It was the night of the fire. I was in the lab. I can hear what they said."',

      },



      stage_E_full_integration: {

        maps_to_game_state: 'RECOVERED (integrated)',

        timeline_real: 'Years (major shifts within 1-3)',

        what_surfaces: 'The memory exists as part of the life narrative — recalled without dissociation, without overwhelming flooding, without loss of temporal orientation.',

        emotional_tone: 'Manageable sadness/grief — not annihilating. Sometimes relief, clarity, even peace.',

        body_response: 'Physiological responses to triggers diminish or resolve (somatic resolution).',

        example_line: '"I remember everything now. It hurts to remember. But I can hold it."',
    ...
    herman_three_stage_model: {

      source: 'Judith Herman, Trauma and Recovery, 1992 — the clinical gold standard',

      iterative_note: 'These stages are ITERATIVE, not strictly sequential. Patients cycle back to earlier phases under stress or when new memories surface — mirror this by allowing game memories to regress.',

      stage_1_safety_stabilization: {

        central_task: 'Establish safety — physical, emotional, relational; regain control over body (sleep, eating, symptoms); build affect-regulation skills.',

        memory_status: 'Memories may begin surfacing but are NOT the focus. Premature memory work is dangerous and contraindicated.',

        duration: 'Weeks to months, sometimes years for complex trauma.',

        game_mapping: 'Corresponds to keeping a memory safely at SUPPRESSED/early FRAGMENTS while the character is stabilized. Rushing deep methods here should backfire.',

      },

      stage_2_remembrance_mourning: {

        central_task: 'Reconstruct the trauma narrative; grieve what was lost. Fragments assembled into coherent narrative and witnessed.',

        memory_status: 'Active memory work — fragments→narrative, emotional processing of recovered content.',

        duration: 'Months to years.',

        game_mapping: 'The FRAGMENTS→SURFACING→RECOVERED arc. Recovery is not just remembering — it is MOURNING.',

      },

      stage_3_reconnection_integration: {

        central_task: 'Reconnect with ordinary life; build a future. Memory metabolized into autobiography; nervous system no longer treats it as present-tense emergency.',

        memory_status: 'Integrated — recalled deliberately without overwhelming response; exists as past, not ongoing emergency.',

        duration: 'Ongoing — "recovery is not a destination but a direction."',

        game_mapping: 'The post_recovery_changes / integration window each memory already defines (the "2-3 day integration" in stage_behaviors is the compressed game version).',
    ...
    body_keeps_the_score: {
      ...
      source: 'Bessel van der Kolk — The Body Keeps the Score (1994 paper; 2014 book); Levine 2010',

      core_thesis: 'When narrative memory fails to encode trauma (hippocampal suppression), the BODY becomes the repository. Trauma is stored as muscular tension patterns, autonomic dysregulation (stuck in fight/flight/freeze), somatosensory flashbacks, and altered interoception.',

      documented_physical_manifestations: [

        'Chronic pain without medical explanation, often in body areas involved in the trauma',

        'Gastrointestinal issues — IBS, nausea, stomach pain (gut-brain axis disruption)',

        'Autoimmune disorders — elevated rates in trauma survivors (ACE Study data)',

        'Cardiovascular — elevated resting heart rate, blood-pressure dysregulation',

        'Chronic fatigue — bodily-held traumatic stress exhausts physiological resources',

        'Muscular tension specific to defensive postures that activated but never completed',

      ],

      psychosomatic_patterns: {

        source: 'HPA-axis / autonomic activation without conscious awareness of the trigger',

        patterns: [

          'Headaches/migraines triggered by environments resembling the trauma location — without recognizing the resemblance',

          'Nausea/vomiting near sensory cues (smells, sounds) tied to the trauma',

          'Nosebleeds and vasomotor symptoms under stress activation (documented in conversion presentations)',

          'Panic attacks in specific locations — the body recognizes danger the conscious mind cannot access',

          'Dermatological reactions — hives, flushing — to trauma triggers',

          'Region-specific pain matching where the body was involved in the original event',
      ...
      conversion_disorder: {
        ...
        dsm5: 'Functional Neurological Symptom Disorder, 300.11 (F44.4-F44.7)',

        source: 'Feinstein 2011; Stone et al. 2009',

        mechanism: 'Genuine neurological symptoms with no organic cause — the motor intention is generated but INHIBITED before execution. A diagnosis of inclusion (positive signs like Hoover\'s sign), not mere exclusion.',

        documented_symptoms: ['Psychogenic blindness', 'Psychogenic paralysis', 'Psychogenic non-epileptic seizures (PNES)', 'Aphonia (loss of voice)', 'Psychogenic anesthesia', 'Psychogenic gait disturbance'],

        game_mapping: 'A character\'s power-specific symptoms (voice loss, vision overwhelm, tremor, time-skip) approaching a suppressed memory are the dramatized cousin of conversion symptoms — worsening near the memory, resolving on integration. The body communicates what the mind cannot access.',

      },

      game_mapping: 'The body_reactions arrays in stage_behaviors and in each memory\'s suppressed_state are the game\'s "body keeps the score" layer. Keep them region-specific and tied to the original event (e.g., throat symptoms for a voice-trauma; wrist pain where a wrist was grabbed).',
    ...
    realistic_timelines: {
      ...
      stabilization_before_memory_work: '2-12 months',

      active_memory_processing: '6-24 months',

      integration_major_shifts: '1-3 years (ongoing after)',

      complex_trauma_total: '3-7 years is common',

      single_event_trauma: 'Weeks to months',

      game_compression_note: 'The engine compresses years into story-days via the PRESSURE_SYSTEM. When writing, preserve the ORDER and NON-LINEARITY even while compressing the clock: body/sensory before emotional before narrative; regressions under stress are realistic and dramatically useful.',
    ...
    non_linearity_rules: {
      ...
      regression: 'New stress, an unsafe disclosure, or a fresh trigger can push a SURFACING memory back toward FRAGMENTS — clinically real and good for pacing.',

      sudden_jumps: 'A matching sensory cue (especially olfactory) or a state-dependent match (frequency/emotional state) can catalyze a sudden jump forward — the "flashback that skips stages."',

      cycling: 'Herman\'s stages are revisited repeatedly; a character can be "integrated" on one memory while destabilized on another.',
  ...
  // MEMORY CLINICAL GROUNDING  ◆ Real-World Enhancement — added 2026-09-07

  // ═══════════════════════════════════════════════════════════════════════════

  // Per-memory clinical notes, keyed by memory id. ADDITIVE lookup layer — does

  // NOT modify any memory object. Join via api.getMemoryWithClinicalGrounding().

  // Each entry ties the specific memory to real trauma-encoding science, the

  // body-first symptom it should show, how it should surface, and the real

  // reliability/risk of its recovery method.



  MEMORY_CLINICAL_GROUNDING: {

    description: 'Per-memory clinical grounding keyed by memory id. See NEUROSCIENCE_NOTES, RECOVERY_METHOD_CLINICAL_BASIS, and REALISTIC_SYMPTOM_PROGRESSION for the general model.',



    kael_mem_01: {

      trauma_type: 'Age-8 event with dissociative sealing; adult-recoverable (>age 7).',

      neuro_mechanism: 'Hippocampal-amygdala split: the FEELING (fear of his own voice) was retained by the amygdala while the hippocampal narrative (the principal, the confession, mother running) was suppressed. Classic retrieval-failure, not erasure.',

      body_first_symptom: 'Throat tightening and chest pressure are region-specific somatic memory (van der Kolk) — the body holds the trauma in the exact system that produced it (voice).',

      surfacing_note: 'Auditory + the exact phrase "stop hurting people" are state-dependent/sensory keys; expect emotional flooding (Stage B) BEFORE the hallway scene assembles (Stage C).',

      recovery_method_reality: 'Primary HYPNOTIC_REGRESSION is CONTESTED in reality (high confabulation risk) — good in-fiction for ambiguity, but GUIDED_MEDITATION/exposure would be the evidence-based route.',

      false_memory_risk: 'Moderate — details produced under regression should be corroborated by Marissa\'s account and Board evidence before treated as fact.',

    },

    kael_mem_02: {

      trauma_type: 'Adult event (age 22); single-incident, fully encodable — recovery can be relatively fast.',

      neuro_mechanism: 'Adult hippocampus intact, but peritraumatic overwhelm fragmented encoding; sensory (standing-wave sensation) hyper-encoded, context thin (Ehlers & Clark perceptual priming).',

      body_first_symptom: 'Somatic resonance/vibration sensations are the SAM sensory trace firing before narrative.',

      surfacing_note: 'FREQUENCY_EXPOSURE = Prolonged-Exposure/EMDR analog: highest-fidelity recall at highest activation cost. Expect full autonomic surge.',

      recovery_method_reality: 'STRONG evidence route (exposure-based) — but requires prior stabilization; unstabilized exposure risks retraumatization.',

      false_memory_risk: 'Low — exposure-based recall of an adult single event is comparatively reliable.',

    },

    kael_mem_03: {

      trauma_type: 'Adult event (age 23), Legacy contact; suppression maintained by ongoing threat.',

      neuro_mechanism: 'State-dependent: encoded under high threat/vigilance; surfaces when a similar wary/hunted state is re-entered.',

      body_first_symptom: 'Hypervigilance, startle, and cold-hands vasomotor signs are autonomic residue.',

      surfacing_note: 'Ambiguity is realistic — surveillance memories blend real cues with paranoia; corroborate against artifacts.',

      recovery_method_reality: 'HYPNOTIC_REGRESSION contested; the physical artifact/warning is the reliable anchor, not the regression content.',

      false_memory_risk: 'Moderate-high given hypnosis + threat priming.',

    },

    kael_mem_04: {

      trauma_type: 'Age-8 CHILDHOOD_VISIT; in-fiction Overtone encoding gives it more vividness than biology alone allows (deliberate supernatural exception).',

      neuro_mechanism: 'Real developmental rule: age-8 material returns fragmentary/body-first. The immature-PFC + amygdala-dominant encoding means overwhelming affect with thin narrative.',

      body_first_symptom: 'Throat closure, cold at the word "STOP", inability to sleep in silence — procedural/somatic patterning typical of childhood trauma encoding.',

      surfacing_note: 'DREAM_INVASION fits: REM bypasses the prefrontal gatekeeper. But dream content is factually UNRELIABLE — symbolic replay, near-replicative nightmare of a PTSD type.',

      recovery_method_reality: 'Dreams are CONTESTED for retrieval; treat surfaced content as a lead to corroborate (Marissa, SWALLOW file), not proof.',

      false_memory_risk: 'Moderate — dream-sourced; requires waking corroboration.',

    },

    kael_mem_05: {

      trauma_type: 'Age-8 CHILDHOOD_VISIT #2 (calibration); deepest layer, requires prior partial awakening.',

      neuro_mechanism: 'State-dependent + childhood encoding: only surfaces after Tier 1-3 fragments exist — clinically mirrors how early material needs scaffolding to approach.',

      body_first_symptom: 'Missing-time and post-event nosebleed pattern; somatic markers stand in for absent narrative.',

      surfacing_note: 'Expect body sensations, emotional states and fragmentary images — NOT a clean scene with dialogue (childhood-encoding rule).',

      recovery_method_reality: 'DREAM_INVASION/high-intensity methods only after stabilization; contested reliability.',

      false_memory_risk: 'Moderate-high; corroborate against Legacy files.',

    },

    mira_mem_01: {

      trauma_type: 'Age-3 assessment; at the edge of episodic-memory capability — largely body/schema, not narrative.',

      neuro_mechanism: 'Age-3 hippocampus barely supports retrievable episodic memory; postnatal neurogenesis overwrites early traces. What remains is emotional schema + somatic state.',

      body_first_symptom: 'Generalized "being measured/watched" dread and body states — not a scene.',

      surfacing_note: 'GUIDED_MEDITATION/body-scan suits preverbal material: lowers defenses so somatic fragments rise. Do NOT expect narrated dialogue from age 3 — flag any as possibly constructed.',

      recovery_method_reality: 'Meditation is MODERATE evidence for lowering barriers; realistic that only schema/feeling returns.',

      false_memory_risk: 'HIGH for any detailed "scene" — age-3 episodic detail is clinically implausible.',

    },

    mira_mem_02: {

      trauma_type: 'Age-16 event; adolescent — largely encodable, PFC still maturing.',

      neuro_mechanism: 'Adolescent encoding near-adult; fragmentation from peritraumatic arousal rather than developmental limits.',

      body_first_symptom: 'Auditory intrusions (Ruth\'s voice) are the SAM sensory trace; olfactory/auditory cues are strongest triggers.',

      surfacing_note: 'HYPNOTIC_REGRESSION contested; a "ghost/voice" motif is exactly the kind of content prone to suggestion — keep ambiguous.',

      recovery_method_reality: 'Prefer corroborable cues; regression content is unreliable.',

      false_memory_risk: 'High under hypnosis.',

    },

    mira_mem_03: {

      trauma_type: 'Age-0 "soul memory"; PURE in-fiction device — biologically NON-recoverable as episodic memory.',

      neuro_mechanism: 'Real rule: infantile amnesia is absolute before ~age 2-3 (neurogenesis overwrites traces). Any narrated age-0 memory is clinically impossible — here it is explicitly the supernatural soul-carried exception.',

      body_first_symptom: 'Only affect/somatic resonance could plausibly carry — never a scene.',

      surfacing_note: 'CROSS_CHARACTER_RESONANCE (group/social contagion) is the fitting mechanic; treat content as shared meaning-making, not retrieved fact.',

      recovery_method_reality: 'Flag in-world: this bypasses real memory science by design; do not present as clinically plausible recall.',

      false_memory_risk: 'N/A clinically (impossible) — narrate as soul-memory, not human episodic memory.',

    },

    mira_mem_04: {

      trauma_type: 'Age-3 CHILDHOOD_VISIT (cataloging); body/schema-dominant.',

      neuro_mechanism: 'Age-3 encoding: emotional schema + somatic state, minimal narrative; requires partial awakening to approach.',

      body_first_symptom: 'Sense of being itemized/inventoried as diffuse dread and body tension.',

      surfacing_note: 'GUIDED_MEDITATION/titration; surfaces as feelings and fragments, not scenes.',

      recovery_method_reality: 'Moderate evidence for barrier-lowering; realistic that narrative stays sparse.',

      false_memory_risk: 'High for any detailed scene at age 3.',

    },

    mira_mem_05: {

      trauma_type: 'Age-5 CHILDHOOD_VISIT (perception test); fragmentary/distorted, blendable with later experience.',

      neuro_mechanism: 'Ages 3-7: potentially recoverable but fragmentary, emotionally distorted, blended with suggestion.',

      body_first_symptom: 'Perceptual oddness/after-images as sensory trace.',

      surfacing_note: 'CASUAL_CONVERSATION can surface a fragment gently (Stage A) — cannot force depth.',

      recovery_method_reality: 'Strong-safe front-end; realistically caps at fragment level.',

      false_memory_risk: 'Moderate-high; corroborate.',

    },

    oren_mem_01: {

      trauma_type: 'Age-9 assessment; late-childhood — some episodic capacity, still fragmentary.',

      neuro_mechanism: 'Age-9 hippocampus more capable than early childhood but stress + developmental limits still fragment encoding.',

      body_first_symptom: 'Time-distortion/déjà-vu sensations as the SAM trace of a temporal event.',

      surfacing_note: 'GUIDED_MEDITATION lowers defenses; expect emotional recovery (Stage B) before scene assembly.',

      recovery_method_reality: 'Moderate evidence; realistic partial return.',

      false_memory_risk: 'Moderate.',

    },

    oren_mem_02: {

      trauma_type: 'Adult (age 24), ER patient event; single-incident adult encoding.',

      neuro_mechanism: 'Adult intact hippocampus; fragmentation from acute overwhelm. Sensory hyper-encoding of the clinical scene.',

      body_first_symptom: 'Nausea/dread on clinical smells (olfactory = strongest trigger, direct limbic access).',

      surfacing_note: 'HYPNOTIC_REGRESSION contested; the medical record is the reliable anchor.',

      recovery_method_reality: 'Prefer corroborable evidence over regression content.',

      false_memory_risk: 'Moderate-high under hypnosis.',

    },

    oren_mem_03: {

      trauma_type: 'Adult (age 25), five-hand manifestation; single dramatic incident.',

      neuro_mechanism: 'Peritraumatic dissociation fragmented an otherwise encodable adult event; strong perceptual priming.',

      body_first_symptom: 'Proprioceptive wrongness / phantom-limb-like sensation as somatic trace.',

      surfacing_note: 'FREQUENCY_EXPOSURE = exposure/EMDR analog: full recall, high activation. Stabilize first.',

      recovery_method_reality: 'STRONG evidence route; reliable recall, high cost.',

      false_memory_risk: 'Low with non-suggestive exposure.',

    },

    oren_mem_04: {

      trauma_type: 'Age-9 CHILDHOOD_VISIT (displacement); requires partial awakening.',

      neuro_mechanism: 'Late-childhood encoding — more scene-capacity than early childhood but still fragmentary and body-first.',

      body_first_symptom: 'Missing-time, spatial disorientation, post-event somatic markers.',

      surfacing_note: 'HYPNOTIC_REGRESSION contested; displacement content especially suggestion-prone — keep ambiguous, corroborate with Legacy files.',

      recovery_method_reality: 'Contested; content is a lead, not proof.',

      false_memory_risk: 'High under regression.',

    },

    oren_mem_05: {

      trauma_type: 'Age-11 CHILDHOOD_VISIT #2 (restaurant disappearance); pre-adolescent, partly encodable.',

      neuro_mechanism: 'Age-11 encoding approaches adolescent capacity; still fragmented by stress + gated behind awakening.',

      body_first_symptom: 'Sudden dread in public/enclosed dining spaces (place-based panic) as autonomic residue.',

      surfacing_note: 'FREQUENCY_EXPOSURE: high-fidelity but high-activation; stabilize first.',

      recovery_method_reality: 'Strong-evidence route with real activation cost.',

      false_memory_risk: 'Low-moderate with non-suggestive exposure.',

    },

    jude_mem_01: {

      trauma_type: 'Age-14 earthquake event; adolescent, largely encodable.',

      neuro_mechanism: 'Adolescent encoding near-adult; fragmentation from acute overwhelm rather than development.',

      body_first_symptom: 'Ground-vibration/rumble somatic trigger; muscle-bracing tension (defensive posture that never completed).',

      surfacing_note: 'GUIDED_MEDITATION/somatic tracking suits the completed-defensive-response angle (Levine SE).',

      recovery_method_reality: 'Moderate evidence; body-based approach apt for a physical-force trauma.',
    ...
    jude_mem_02: {
      ...
      trauma_type: 'Adult (age 20), prison wall; single dramatic incident.',

      neuro_mechanism: 'Adult encoding; peritraumatic arousal fragmented context, hyper-encoded the physical sensation of force.',

      body_first_symptom: 'Somatic memory of strain/pressure in the exact muscle groups involved.',

      surfacing_note: 'HYPNOTIC_REGRESSION contested; physical evidence (the wall, witnesses) is the anchor.',

      recovery_method_reality: 'Prefer corroborable evidence.',
    ...
    jude_mem_03: {
      ...
      trauma_type: 'Adult (age 25), held a building; single overwhelming incident.',

      neuro_mechanism: 'Peritraumatic dissociation split feeling from narrative; strong perceptual priming of the moment.',

      body_first_symptom: 'Whole-body load/strain sensation as somatic trace; autonomic surge on structural rumble.',

      surfacing_note: 'FREQUENCY_EXPOSURE: exposure/EMDR analog; full recall, high activation. Stabilize first.',

      recovery_method_reality: 'Strong-evidence route; reliable, costly.',
    ...
    jude_mem_04: {
      ...
      trauma_type: 'Age-14 CHILDHOOD_VISIT (full truth); the "fight"; deepest layer.',

      neuro_mechanism: 'Adolescent CV: more scene-capacity than early childhood, but the deepest sealing needs prior awakening to approach.',

      body_first_symptom: 'Defensive-posture tension and startle; region-specific pain where the body braced/struck.',

      surfacing_note: 'NEAR_DEATH_EXPERIENCE = strictly a narrative event (REDs / gamma-surge life review). 100% flood, life-threatening; never a therapy.',

      recovery_method_reality: 'Speculative/experimental in reality; use only as the dramatized all-locks-release event.',

      false_memory_risk: 'Mixed/unknown — content richness ≠ accuracy.',

    },

    jude_mem_05: {

      trauma_type: 'Age-15 CHILDHOOD_VISIT (freeway crack); adolescent, largely encodable but sealed.',

      neuro_mechanism: 'Age-15 near-adult encoding; sealing maintained by threat, gated behind awakening.',

      body_first_symptom: 'Panic on highways/overpasses (place-based) as autonomic residue.',

      surfacing_note: 'CASUAL_CONVERSATION can surface a fragment gently; cannot force the full sealed event.',

      recovery_method_reality: 'Safe front-end; realistically fragment-level only.',
    ...
    zara_mem_01: {
      ...
      trauma_type: 'Not classic amnesia — Zara KNOWS but conceals; "slip-ups" reveal a maintained cover, not a suppressed memory.',

      neuro_mechanism: 'Different mechanism: intact memory under deliberate/─trained concealment. The tells are cognitive-load leakage, not SAM intrusions.',

      body_first_symptom: 'Micro-inconsistencies and delayed responses under load — not somatic flashbacks.',

      surfacing_note: 'CASUAL_CONVERSATION surfaces contradictions; this is detective work, not trauma recovery.',

      recovery_method_reality: 'Note the CONTRAST: use to teach players that not all hidden memory is dissociative amnesia.',

      false_memory_risk: 'Low — the memory is intact; the risk is her deception, not confabulation.',

    },

    zara_mem_02: {

      trauma_type: 'Timeline/age contradictions; concealment, not suppression.',

      neuro_mechanism: 'Intact memory, controlled disclosure; contradictions are the seams of a constructed cover story.',

      body_first_symptom: 'No somatic-memory layer expected — behavioral tells only.',

      surfacing_note: 'CASUAL_CONVERSATION; logical contradiction, not sensory flashback.',

      recovery_method_reality: 'Contrast case to genuine dissociative amnesia elsewhere in the engine.',

      false_memory_risk: 'Low.',

    },

    zara_mem_03: {

      trauma_type: 'Adult (age 25), full alien reveal; here genuine dissociative sealing of an overwhelming event.',

      neuro_mechanism: 'Peritraumatic dissociation of an event too vast to encode narratively; amygdala-dominant terror with thin context.',

      body_first_symptom: 'Dissociative/depersonalization sensations, autonomic surge on specific cues.',

      surfacing_note: 'HYPNOTIC_REGRESSION contested — extraordinary content is maximally suggestion-prone; keep deliberately ambiguous.',

      recovery_method_reality: 'Content unreliable; corroborate against physical/Legacy evidence.',

      false_memory_risk: 'Very high — the archetypal "recovered under hypnosis" caution case.',

    },

    zara_mem_04: {

      trauma_type: 'Age-5 CHILDHOOD_VISIT (preparation); body/schema-dominant.',

      neuro_mechanism: 'Ages 3-7 encoding: fragmentary, distorted, blended; requires awakening to approach.',

      body_first_symptom: 'Diffuse "being readied/handled" dread and body tension — not a scene.',

      surfacing_note: 'CROSS_CHARACTER_RESONANCE (shared childhood) fits; expect divergent versions across characters that may be irreconcilable.',

      recovery_method_reality: 'Moderate group-dynamic evidence; watch collaborative-recall contamination.',

      false_memory_risk: 'High for detail; moderate-high via social contagion.',

    },

    zara_mem_05: {

      trauma_type: 'Age-9 CHILDHOOD_VISIT (basement signal); late-childhood, partly encodable, sealed.',

      neuro_mechanism: 'Age-9 encoding with more scene-capacity but still fragmentary; Legacy-surveillance overlay.',

      body_first_symptom: 'Auditory/subsonic-signal sensations and place-based dread (basements) as sensory + autonomic traces.',

      surfacing_note: 'GUIDED_MEDITATION lowers defenses; auditory cues are strong keys. Corroborate against Legacy files.',

      recovery_method_reality: 'Moderate evidence; realistic partial return.',

      false_memory_risk: 'Moderate — corroborate.',
  ...
  api: {
    ...
    // ── Real-World Enhancement accessors (added 2026-09-07) ──────────────────

    getNeuroscienceNotes() {

      return this._parent.NEUROSCIENCE_NOTES || null;

    },



    getMethodClinicalBasis(methodId) {

      var cb = this._parent.RECOVERY_METHOD_CLINICAL_BASIS || {};

      return methodId ? (cb[methodId] || null) : cb;
    ...
    getSymptomProgression() {
      ...
      return this._parent.REALISTIC_SYMPTOM_PROGRESSION || null;

    },



    getMemoryClinicalGrounding(memoryId) {

      var g = this._parent.MEMORY_CLINICAL_GROUNDING || {};

      return memoryId ? (g[memoryId] || null) : g;

    },



    getMemoryWithClinicalGrounding(memoryId) {

      var mem = this.getMemory(memoryId);

      if (!mem) return null;

      var grounding = this.getMemoryClinicalGrounding(memoryId);

      var primary = (mem.recovery_conditions && mem.recovery_conditions.primary_method) || null;

      return {

        memory: mem,

        clinical_grounding: grounding,

        method_clinical_basis: primary ? this.getMethodClinicalBasis(primary) : null,
...
if (typeof window !== 'undefined') { try { window.MEMORY_RECOVERY_ENGINE = MEMORY_RECOVERY_ENGINE; } catch(e){} }
